...
const include = { officials: true, requirements: true } satisfies Prisma.ReliefCampInclude;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const where: Prisma.ReliefCampWhereInput = {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const camp = await prisma.reliefCamp.findUnique({ where: { id: req.params.id }, include });
    if (!camp) throw ApiError.notFound("Relief camp not found");
...
router.put(
  ...
  authenticate,
  requireRole("ADMIN"),
  validate({ params: idParamSchema, body: campBodySchema }),
  asyncHandler(async (req, res) => {
    const { officials, requirements, ...data } = req.body;
    const existing = await prisma.reliefCamp.findUnique({ where: { id: req.params.id } });
    if (!existing) throw ApiError.notFound("Relief camp not found");

    const camp = await prisma.$transaction(async (tx) => {
      await tx.campOfficial.deleteMany({ where: { campId: req.params.id } });
      await tx.campRequirement.deleteMany({ where: { campId: req.params.id } });
...
router.delete(
  ...
  validate({ params: idParamSchema }),
  asyncHandler(async (req, res) => {
    await prisma.reliefCamp.delete({ where: { id: req.params.id } }).catch(() => {
      throw ApiError.notFound("Relief camp not found");
    });
    res.status(204).send();
  })
);
