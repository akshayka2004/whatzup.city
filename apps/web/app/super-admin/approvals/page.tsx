...
export default function SuperAdminApprovalsPage() {
  ...
  const getRoleIcon = (type: string) => {
    switch (type) {
      case 'BUSINESS': return <Building2 className="h-4 w-4 text-cyan-400" />;
      case 'INFLUENCER': return <Sparkles className="h-4 w-4 text-violet-400" />;
      case 'PROFESSIONAL': return <Briefcase className="h-4 w-4 text-emerald-400" />;
      case 'ORGANIZATION': return <Heart className="h-4 w-4 text-rose-400" />;
      case 'GOVERNMENT': return <ShieldAlert className="h-4 w-4 text-amber-400" />;
      case 'EVENT_ORGANIZER': return <Calendar className="h-4 w-4 text-fuchsia-400" />;
                      ...
                      <div className="space-y-2 flex-1">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-500 font-mono">Request ID: {req.id}</p>
                          </div>
                        </div>
                    ...
                    {reviewingItem.entity?.type === 'BUSINESS' && reviewingItem.entity?.business && (
                      <div className="space-y-2 text-xs">
                        {[
                          ...
                          <div key={label} className="grid grid-cols-3 py-1 border-b border-white/5">
                            <span className="text-slate-500">{label}</span>
                            <span className="col-span-2 text-slate-200 font-semibold">{val}</span>
                          </div>
                        ))}
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                      {(reviewingItem.entity?.documents || []).length === 0 ? (
                        <p className="text-xs text-slate-500 italic py-4 text-center">No documents uploaded.</p>
                      ) : (
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                <FileText className={`h-5 w-5 ${isSelected ? 'text-violet-400' : 'text-slate-450'}`} />
                                <div className="min-w-0">
                                  <p className="text-xs font-semibold truncate max-w-[170px]">{doc.name}</p>
                                  <p className="text-[9px] text-slate-500">{doc.documentType || 'PROOF'}</p>
                                </div>
                              </div>
                              <a
                                href={doc.fileUrl}
                                ...
                                className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-cyan-400 hover:bg-cyan-500/10"
                              >
                  ...
                  {selectedDoc ? (
                    <div className="h-full flex flex-col">
                      <div className="border-b border-white/5 pb-2 mb-3 flex items-center justify-between shrink-0">
                        <div>
                          <h4 className="text-xs font-bold text-slate-200 truncate max-w-[320px]">{selectedDoc.name}</h4>
                          <p className="text-[9px] text-slate-400 font-mono mt-0.5">{selectedDoc.documentType || 'VERIFICATION_CREDENTIAL'}</p>
                        </div>
                        {selectedDoc.fileUrl && selectedDoc.fileUrl !== '#' && (
                          <a href={selectedDoc.fileUrl} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-400 hover:underline flex items-center gap-1 font-semibold">
                            Fullscreen <ExternalLink className="h-3 w-3" />
                          </a>
                        )}
                      </div>
                      <div className="flex-1 overflow-y-auto pr-1 flex items-center justify-center bg-black/40 rounded-xl border border-white/5 p-4">
                        {(() => {
                          ...
                          return (
                            <div className="w-full h-full min-h-[300px] bg-slate-900 border border-slate-700/50 rounded-lg p-6 flex flex-col justify-between font-serif relative overflow-hidden select-none">
                              ...
                              <div className="border-b-2 border-amber-500/20 pb-4 text-center">
                                <h5 className="text-sm font-bold tracking-wider text-amber-500 uppercase">State Registry Authority</h5>
                                <p className="text-[9px] text-slate-400 uppercase tracking-widest font-sans mt-1">Official Credential Certificate</p>
                              </div>
                              <div className="space-y-4 my-4 font-sans text-left text-[11px] text-slate-350">
                                <div className="grid grid-cols-3 gap-2">
                                  <span className="text-slate-500 font-semibold">Owner Entity:</span>
                                  <span className="col-span-2 text-slate-100 font-bold">{reviewingItem.entity?.name}</span>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                                  <span className="text-slate-500 font-semibold">License / Document ID:</span>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                  <span className="text-slate-500 font-semibold">Registered On:</span>
                                  <span className="col-span-2 text-slate-200">{new Date(reviewingItem.createdAt).toLocaleDateString()}</span>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                              ...
                              <div className="border-t border-slate-700/50 pt-4 flex justify-between items-center text-[8px] font-sans text-slate-500">
                                ...
                                <div className="h-8 w-8 rounded-full border border-amber-500/20 bg-amber-500/5 flex items-center justify-center text-amber-500/45 text-[9px] font-bold">SEAL</div>
                              </div>
                            </div>
                          );
                        })()}
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-slate-500">
                      ...
                      <p className="text-xs font-bold text-slate-400">No Document Selected</p>
                      <p className="text-[10px] text-slate-500 mt-1 max-w-[240px]">Click any credential on the left to preview it here.</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        {approvingItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <Card className="w-full max-w-sm p-6 rounded-2xl border border-white/10 bg-[#0c0c10] shadow-2xl relative text-center">
              <button onClick={() => setApprovingItem(null)} className="absolute top-4 right-4 text-slate-500 hover:text-slate-350 cursor-pointer">
                <X className="h-5 w-5" />
              </button>
              ...
              <p className="text-xs text-slate-400 mb-4 leading-relaxed">
                Confirm approval of <span className="font-semibold text-slate-200">"{approvingItem.entity?.name}"</span>.
              </p>
              <div className="space-y-3 mb-6 text-left">
                <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest block">Moderator Notes</label>
                <Input
              ...
              <div className="flex justify-center gap-3">
                <Button onClick={() => setApprovingItem(null)} variant="outline" className="rounded-xl border-white/10 hover:bg-white/5 text-slate-300 px-4 cursor-pointer">
                  Cancel
                </Button>
                <Button onClick={handleApprove} disabled={actionLoading} className="rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white px-4 cursor-pointer flex items-center gap-1.5">
        ...
        {rejectingItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <Card className="w-full max-w-md p-6 rounded-2xl border border-white/10 bg-[#0c0c10] shadow-2xl relative">
              <button onClick={() => setRejectingItem(null)} className="absolute top-4 right-4 text-slate-500 hover:text-slate-350 cursor-pointer">
                <X className="h-5 w-5" />
              </button>
              <h3 className="text-xl font-bold text-slate-100 mb-2">Reject Submission</h3>
              <p className="text-xs text-slate-400 mb-4 leading-relaxed">
                Provide a reason for rejecting <span className="font-semibold text-slate-200">"{rejectingItem.entity?.name}"</span>.
              </p>
              <div className="space-y-4">
                <div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <Button onClick={() => setRejectingItem(null)} variant="outline" className="rounded-xl border-white/10 hover:bg-white/5 text-slate-350 cursor-pointer">
                    Cancel
                  </Button>
                  <Button onClick={handleReject} disabled={!rejectReason || actionLoading} className="rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-semibold cursor-pointer flex items-center gap-1.5">
                    {actionLoading && <Loader2 className="h-3 w-3 animate-spin" />}
                    Confirm Rejection
                  </Button>
                </div>
